...
fun <T> DocumentBuilderApi.table(block: TableBuilderApi<T>.() -> Unit) = bind(TableBuilderApi<T>().apply(block))

fun <T> SheetBuilderApi.table(block: TableBuilderApi<T>.() -> Unit) = bind(TableBuilderApi<T>().apply(block))
 ...
 */
@TabulateMarker
class TableBuilderApi<T> internal constructor() : ModelBuilderApi<Table<T>, TableBuilderState<T>>(
    TableBuilderState()) {

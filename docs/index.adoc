...
----
`AttributeOperation` is next level abstraction. It is used to render each attribute from `AttributedContext` context. The `AttributeOperation` dispatching can be achieved by `AttributesHandlingOperation` wrapper that is responsible for resolving all `AttributeOperations` and matching them with corresponding attributes.
...
Order of appearance:
 ...
 - `TableStart` and `OpenTableOperation`,
 - `ColumnStart` and `OpenColumnOperation`,
 - `RowStart` and `OpenRowOperation`,
 - `CellContext` and `RenderRowCellOperation`,
 - `RowEnd` and `CloseRowOperation`,
 - `TableEnd` and `CloseTableOperation`.

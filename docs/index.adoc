...
Library sole purpose is to provide means for data sets exporting. This job is realised through simple, intuitive pattern of a template class dispatching workloads to managed, pluggable operations.
A template which is referred to as `TabulationTemplate` iterates through lazy `RowContextResolver` as row index requests are issued by `TabulationApi` exposed to consumers.
Consumer interaction with library may go through `TabulationApi` and then it looks like this:

 - *declare table model* through DSL (or java fluent) builder,
 - *enqueue a collection element* (or enqueue nothing as one is exporting only custom rows). Adding new collection element will enable it for row context resolution. `RowContext` is centric operational entity which is passed to third party operation implementations in order to supply information required by rendering context.

 - *request next row rendering*,
...
<<<
=== Synthetic row resolution algorithm.

<<<
=== Row context resolution algorithm.

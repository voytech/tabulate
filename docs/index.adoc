...
----
<1> Define the `RenderingContext` class this export operation provider is compatible with,
<2> Instantiate new `RenderingContext` instance. This instantiation occurs always at the very beginning.
<3> Declare compatible `TabulationFormat`. It is a descriptor consisting from file extension string, and from export operation provider identifier.
<4> This is the most important step. Here we can implement actual table rendering logic.

...
----
`AttributeOperation` is next level abstraction. It is used to render each attribute from `AttributedContext` context. The `AttributeOperation` dispatching can be achieved by `AttributesHandlingExportOperation` wrapper that is responsible for resolving all `AttributeOperations` and matching them with corresponding attributes.

...
class ExcelTableAttributesOperations: AttributeOperationsFactory<ApachePoiRenderingContext, Table<*>> {
 ...
 * @since 0.1.0
 */
class ExcelTableOperations : ExportOperationsFactory<ApachePoiRenderingContext, Table<*>>() {

    override fun getDocumentFormat(): DocumentFormat<ApachePoiRenderingContext> =
        format("xlsx", "poi")

    override fun provideExportOperations(): OperationsBuilder<ApachePoiRenderingContext, Table<*>>.() -> Unit = {
        ...
        operation(OpenRowOperation { renderingContext, context ->
            renderingContext.provideRow(context.getSheetName(), context.getRow())
        })
        ...
        operation(CloseRowOperation { _,_ ->})
        operation(CloseColumnOperation { _, _ ->})
        operation(CloseTableOperation { _, _ ->  })
    }

    override fun getAttributeOperationsFactory(): AttributeOperationsFactory<ApachePoiRenderingContext, Table<*>> =
    ...
    private fun ApachePoiRenderingContext.provideCell(context: CellContext, block: (SXSSFCell.() -> Unit)) =
        provideCell(context.getSheetName(), context.getRow(), context.getColumn()) { it.apply(block) }

    private fun ApachePoiRenderingContext.renderImageDataCell(context: CellContext) {
        createImageCell(
            context.getSheetName(), context.getRow(), context.getColumn(),
